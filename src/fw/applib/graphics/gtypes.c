...
// Per-channel lookup table for 33% blending (16 bytes instead of 4096 bytes)
// s_blend_33_channel_lut[src][dest] = round((2*dest + src) / 3)
// 66% blending can be achieved by swapping src and dest
static const uint8_t s_blend_33_channel_lut[4][4] = {
  // dest: 0  1  2  3
  {0, 1, 1, 2},  // src=0
  {0, 1, 2, 2},  // src=1
  {1, 1, 2, 3},  // src=2
...
// Helper function to blend two colors at 33% using per-channel lookup
static inline GColor8 prv_blend_33_percent(GColor8 src_color, GColor8 dest_color) {
  return (GColor8) {
    .r = s_blend_33_channel_lut[src_color.r][dest_color.r],
    .g = s_blend_33_channel_lut[src_color.g][dest_color.g],
    .b = s_blend_33_channel_lut[src_color.b][dest_color.b],
    .a = 3,  // Fully opaque result
  };
}
...
GColor8 gcolor_blend(GColor8 src_color, GColor8 dest_color, uint8_t blending_factor) {
  switch (blending_factor) {
    case 0:
      ...
      return dest_color;
    case 1:
      // 33% blend using per-channel lookup (saves 4KB vs full lookup table)
      return prv_blend_33_percent(src_color, dest_color);
    case 2:
      // 66% blend - same as 33% with src/dest swapped
      return prv_blend_33_percent(dest_color, src_color);
    case 3:
      // Fast path: 100%
      return src_color;
    default:
